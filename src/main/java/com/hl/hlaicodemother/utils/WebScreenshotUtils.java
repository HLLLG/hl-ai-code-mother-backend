...
public class WebScreenshotUtils {
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            // 创建临时目录
            String rootDir = System.getProperty("user.dir") + File.separator + "tmp" + File.separator + "screenshots";
            FileUtil.mkdir(rootDir);
            // 图片后缀
            String IMAGE_SUFFIX = ".png";
            // 原始截图文件路径
            String imageSavePath = rootDir + UUID.randomUUID().toString().substring(0, 8) + IMAGE_SUFFIX;
            // 访问网页
            ...
            String compressImageSavePath = rootDir + File.separator + RandomUtil.randomNumbers(5) + COMPRESS_SUFFIX;
    ...
    private static void saveImage(byte[] imageBytes, String filePath) {
        try {
            ...
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "保存图片失败: " + filePath);
        }
    }
     ...
     */
    private static void compressImage(String sourceFilePath, String outputFilePath) {
        try {
            final float quality = 0.3f;
            ImgUtil.compress(FileUtil.file(sourceFilePath), FileUtil.file(outputFilePath), quality);
        } catch (Exception e) {
            log.error("压缩图片失败:{} -> {}", sourceFilePath, outputFilePath, e);
